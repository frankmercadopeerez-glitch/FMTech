...
// ============================================
// CUSTOM CURSOR (solo desktop, no touch)
// ============================================
(function initCustomCursor() {
  ...
  style.textContent = `
    *, *::before, *::after { cursor: none !important; }
    ...
    body:has(a:hover) #fm-cursor-dot,
    body:has(button:hover) #fm-cursor-dot { background: #06b6d4; transform: translate(-50%, -50%) scale(1.5); }
    body:has(a:hover) #fm-cursor-ring,
    body:has(button:hover) #fm-cursor-ring { width: 52px; height: 52px; border-color: rgba(6,182,212,0.5); }
  `;
  document.head.appendChild(style);

  const dot = document.createElement("div");
  dot.id = "fm-cursor-dot";
  const ring = document.createElement("div");
  ring.id = "fm-cursor-ring";
